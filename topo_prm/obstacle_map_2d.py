...
Verbatim copy of mppi_src/envs/obstacle_map_2d.py so the topo_prm package is
self-contained. The TopoPRM planner reads the numpy occupancy grid
(``ObstacleMap._map``) directly, so torch is not on its hot path.
"""
...
# torch is only needed for the GPU collision helpers (compute_cost /
# convert_to_torch), which the TopoPRM planner never calls. Make it optional so
# this module also imports in the `odp` conda env (which has no torch) for the
# odp reachable-set backend.
try:
    import torch
...
class ObstacleMap:
    ...
    def __init__(
        ...
        cell_size: (m)
        """
        # device and dtype (torch-only; harmless when torch is absent)
        if _HAS_TORCH:
            want_cuda = device == torch.device("cuda")
        ...
        if _HAS_TORCH:
            ...
            self._torch_cell_map_origin = None
    ...
    def convert_to_torch(self):
        assert _HAS_TORCH, "convert_to_torch requires torch (not installed)"
        self._map_torch = torch.from_numpy(self._map).to(self._device, self._dtype)
        return self._map_torch

    def clear(self) -> None:
        """Remove all obstacles, resetting to an empty map.

        Used to start with no *known* obstacles when obstacles are revealed
        incrementally (e.g. by a lidar sensor).
        """
        self._map.fill(0.0)
        self.circle_obs_list = []
        self.rectangle_obs_list = []
        if self._map_torch is not None:
    ...
    def compute_cost(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        assert _HAS_TORCH, "compute_cost requires torch (not installed)"
...
def generate_random_obstacles(
    ...
    max_iteration: int,
    seed: int,
    keepout_circles: List[Tuple[float, float, float]] = None,
) -> None:
    """
    Generate random obstacles.

    keepout_circles: optional list of (x, y, radius) regions that obstacles
        must not intersect (e.g. safe regions).
    """
    rng = np.random.default_rng(seed)
    if keepout_circles is None:
        keepout_circles = []
    ...
    for i in range(num_circle_obs):
        ...
        while num_trial < max_iteration:
            ...
            # reject if intersecting a keep-out region (e.g. safe region)
            for kx, ky, kr in keepout_circles:
                if np.linalg.norm(np.array([kx, ky]) - center) <= kr + radius:
                    is_overlap = True
                    break
