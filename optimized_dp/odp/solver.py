...
def HJSolver(dynamics_obj, grid, multiple_value, tau, compMethod,
             ...
             computeTimeToReach=False, verbose=True):

    if verbose: print("Welcome to optimized_dp \n")
    ...
    # Time-varying target sets: detect trailing time dim on `target`
    target_dim = target.ndim
    target_i = target[..., 0] if target_dim > grid.dims else target

    if constraint is None:
        if verbose: print("No obstacles set !")
        init_value = target_i
    else:
        if verbose: print("Obstacles set exists !")
        ...
        init_value = np.maximum(target_i, -constraint_i)
    ...
    iter = 0
    tNow = tau[0]
    if verbose: print("Started running\n")
    ...
    for i in range (1, len(tau)):
        ...
        # taking target at each timestep
        if target_dim > grid.dims:
            target_i = target[..., i]
            l0 = hcl.asarray(target_i)
        ...
        while tNow <= tau[i] - 1e-4:
            ...
            if untilConvergent is True:
                ...
                if verbose: print("Max difference between V_old and V_new : {:.5f}".format(diff))
                if diff < epsilon:
                    if verbose: print("Result converged ! Exiting the compute loop. Have a good day.")
                    break
