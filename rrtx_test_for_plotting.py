...
class SFF_star:
    ...
    def init_trees(self, showPlot=True):        
        ...
        # ----------------------------------------------------------
        # Initialization (SAFE for domain [-25,25]^2)
        # ----------------------------------------------------------
        self.hjr_fno.safe_regions = [
        ...
        phi_i = self.hjr_fno.feasible_region[0]
        phi_j = self.hjr_fno.feasible_region[1]

        center_i = self.hjr_fno.safe_regions[0].copy()
        center_j_original = self.hjr_fno.safe_regions[1].copy()

        # ----------------------------------------------------------
        # Bisection search for maximum separation
        # ----------------------------------------------------------
        d_min = 0.0
        d_max = 20.0      # upper bound (must exceed expected max)
        tol = 1e-3
        max_iter = 40
        ...
        for _ in range(max_iter):
            ...
            test_center_j = center_j_original.copy()
            test_center_j[0] = center_i[0] + d_mid
            ...
            if valid:
                best_d = d_mid
                best_ball_center = ball_center
                d_min = d_mid
            else:
                d_max = d_mid

            if abs(d_max - d_min) < tol:
                break

        print("Maximum allowable x-distance:", best_d)

        # ----------------------------------------------------------
        # Update safe_regions with optimal configuration
        # ----------------------------------------------------------
        self.hjr_fno.safe_regions[1][0] = center_i[0] + best_d

        print("Updated safe_regions:")
        print(self.hjr_fno.safe_regions)

        # ----------------------------------------------------------
        # Plot δ-ball
        # ----------------------------------------------------------
        if best_ball_center is not None:
        ...
        self.plotting.safe_regions = self.hjr_fno.safe_regions
        # -------------------------------------------------
        ...
        #Save file
        #=====
        # filename = os.path.join("HJR_FNO/figures", f"TSP_RRTX_known.svg")
    ...
    def planning(self, hamiltonian_cycle=False, showPlot=True):
        ...
        self.ax.scatter(
            self.plotting.xG[goal_ids][0], self.plotting.xG[goal_ids][1],
            marker='*',
            s=300,                    # size (adjust as needed)
            c='red',         # face color
            edgecolors='black',       # outline color
        ...
        for i in range(len(self.hjr_fno.safe_regions)):
            ...
            mat_data = loadmat(f'/home/kmuenpra/git/HJR-FNO-ContingencyPlanning/test/HJB_training_mat/true_data{i} 1.mat')
            reachable_set = mat_data['data_safe']
            print(reachable_set.shape)
        
            theta_slice = np.argmin(np.abs(self.hjr_fno.theta_array_fine - self.rrtx_trees[id].robot_state[2]))
            reachable_set_slice = reachable_set[..., theta_slice, -1]
        ...
        mat_data = loadmat(f'/home/kmuenpra/git/HJR-FNO-ContingencyPlanning/test/HJB_training_mat/true_data0 1.mat')
        V_true = mat_data['data_safe'][...,-1]
        V_pred = self.hjr_fno.HJR_sets[0][...,-1].cpu().numpy()
        
        print(V_pred.shape)
        print(V_true.shape)
        
        dx_pred = self.hjr_fno.g.vs[0][1] - self.hjr_fno.g.vs[0][0]
        dy_pred = self.hjr_fno.g.vs[1][1] - self.hjr_fno.g.vs[1][0]
        dth_pred = self.hjr_fno.g.vs[2][1] - self.hjr_fno.g.vs[2][0]

        dx_true = self.hjr_fno.g_fine.vs[0][1] - self.hjr_fno.g_fine.vs[0][0]
        dy_true = self.hjr_fno.g_fine.vs[1][1] - self.hjr_fno.g_fine.vs[1][0]
        dth_true = self.hjr_fno.g_fine.vs[2][1] - self.hjr_fno.g_fine.vs[2][0]
        

        # True surface V=0
        verts_true, faces_true, _, _ = marching_cubes(
            V_true,
            level=0,
            spacing=(dx_true, dy_true, dth_true)
        )

        # Pred surface V=eps
        eps = 0.02
        verts_pred, faces_pred, _, _ = marching_cubes(
            V_pred,
            level=-0.6155,
            spacing=(dx_pred, dy_pred, dth_pred)
        )
        
        verts_pred[:, 0] += self.hjr_fno.g.vs[0][0]
        verts_pred[:, 1] += self.hjr_fno.g.vs[1][0]
        verts_pred[:, 2] += self.hjr_fno.g.vs[2][0]

        verts_true[:, 0] += self.hjr_fno.g_fine.vs[0][0]
        verts_true[:, 1] += self.hjr_fno.g_fine.vs[1][0]
        verts_true[:, 2] += self.hjr_fno.g_fine.vs[2][0] + 1e-3

        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        
        # Pred (dark blue, alpha 1)
        ax.plot_trisurf(
            verts_pred[:, 0],
            verts_pred[:, 1],
            faces_pred,
        ...
        # Top view, slightly tilted
        ax.view_init(elev=46, azim=-85, roll=1)
        ax.set_xticks([-10, 0 ,10])
        ax.set_xticklabels([r"-10", r"0", r"10"])
    ...
    def generate_random_obstacles(self, env, N, r_min=1, r_max=1.5,
                               min_dist_between=2,
                               goals=None, min_dist_to_goal=2.5,
                               origin_safe_radius=3.0,
                               start_max_radius=7.5,
                               start_min_dist_to_obs=1,
                               max_attempts=1000):
        """
    ...
    def test_case_contingency_plan(self, _fig=None, _ax=None, num_obs=1, special_case=False):
        ...
        if not special_case:
            ...
            unknown_obs = obs_list #[obs_list[i] for i in indices[n_known:]]
    ...
    def check_sets_overlap(self):
        ...
        for i in range(len(self.hjr_fno.safe_regions) - 1):
            ...
            center_i = self.hjr_fno.safe_regions[i].copy()
            center_j_original = self.hjr_fno.safe_regions[j].copy()

            # -------------------------------------------------
            # Bisection parameters
            # -------------------------------------------------
            d_min = 0.0
            d_max = 20.0      # choose sufficiently large search bound
            tol = 1e-3
            max_iter = 40
            ...
            for _ in range(max_iter):
                ...
                test_center_j = center_j_original.copy()
                test_center_j[0] = center_i[0] + d_mid   # shift along x
                ...
                if valid:
                    best_d = d_mid
                    best_center = ball_center
                    d_min = d_mid   # try larger separation
                else:
                    d_max = d_mid   # reduce separation
            ...
            if best_center is not None:
                ...
                print("No δ-clear overlap exists even at zero separation.")
...
def main():
    ...
    with open(output_path, "a") as f:
        ...
        if not file_exists:
            f.write("iter,TReach,success,V_val,g_val,ham_term,reach_goal\n")

        for k in range(num_iter):
        

            _fig, _ax, TReach, success, V_val, g_Val, ham_term = sff.test_case_contingency_plan(
                _fig, _ax, num_obs=num_obs, special_case=True
            )
            
            if TReach == 999: #skip the case where the robot spawn outside
                continue
            else:
